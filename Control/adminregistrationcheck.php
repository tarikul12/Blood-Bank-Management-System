            ...
            else
            {
                if ((move_uploaded_file($_FILES["picture"]["tmp_name"], "../uploads/profile_picture/" . $_FILES["picture"]["name"])) )
                {
                    ...
                    else
                    {
                        $mydb = new db();
                        $myconn = $mydb->openConn();
                        $mydb->insertapplicant($fname, $lname, $uname, $email, $nid, $phone, $password, $cpassword, $imgpath,     "applicantofadmin", $myconn);
                        $mydb->insertapplicant_for_backup($fname, $lname, $uname, $email, $nid, $phone, $password, $cpassword, $imgpath,  "admin_back_up", $myconn);
